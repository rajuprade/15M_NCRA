...
extern long  cmdTimer[MAXDEVCMD];
 ...
 void devSentinal(parseCMSCmd *cptr, int *chkexec) 
 {
         ...
         if(!strcasecmp(cptr->cmdelem.cmdname, periodicCmd[DOMON]))  {
                ...
                *chkexec = 1;
         } else if (!strcasecmp(cptr->cmdelem.cmdname, devSentinalCmd[INIT])) // strcasecmp ---> compare two strings ignoring case
             {
               printCmdPkt(cptr);             
