...
BYTE  txd_char_len,rxd_char_len;
     ...
     switch (txm_stt)
       {
           ...
           case IDLE:  if (req_frameq.hd== NULL) 
		       ...
		       else
		         {
			       ...
			       compile();
			       //printf("{%d %d %d %s} ",tx_buff->src_task_id,tx_buff->des_task_id,tx_buff->length,tx_buff->data);
           ...
           case XMIT:    txm_stt = TXM_FREE;
			 ...
			 b = write(clientfd,txd_char,txd_char_len);

			 //printf("No of bytes written: %d",b);
...
void readSock(void)
{
  ...
  n = read(clientfd,rxd_char,sizeof(rxd_char));
  ...
  printf(" RXD CHAR : %d",strlen(rxd_char));
    for(i=0;i<strlen(rxd_char);i++)
        printf(" data is as follow  %02x ",rxd_char[i]);
...
void rx_mc()
{
  ...
  { 
    switch (rxm_stt)
    {
      ...
      case GET_SRC_ID: //printf("B\n");
                       rx_buff->src_task_id = rxd_char[i++];
		       //printf("SRC: %02x",rx_buff->src_task_id);
		       rxm_stt = GET_DES_ID;
                       break;
      case GET_DES_ID: //printf("C\n");
	               rx_buff->des_task_id = rxd_char[i++];
		       //printf("DES: %02x",rx_buff->des_task_id);
		       rxm_stt = ASMBL;
                       break;
