     ...
     if(FreqSet[1]<110000 || FreqSet[1]>240000) {
       ...
       for(i=0;i<2;i++)
          if( pattern('K',FreqSet[i],i ) != 0 ) {
            fprintf(stderr, "%s : Error in Setting\n",argv[0]);  
          } else { fprintf(stderr, "%s : Successfully Set \n",argv[0]); }
...
void SetBit(int data[],int ref,int cnt)
{
      ...
      portStatus=ioperm(baseaddress,3,1);//On success it returns zero
        if(portStatus<0) { fprintf(stderr,"Parallel Port Access denied %d",portStatus); exit(1);}
  ...
  if (ioperm(baseaddress, 3, 0)) {perror("ioperm"); exit(1);}
}
...
{                 
	        ...
	        for(i=0;i<m;i++)
		{
			...
			for(s=0;s<8;++s)
                 ...
                 #endif      
     SetBit(f,72,count); //Here 72 is fixed for binary bit length.
