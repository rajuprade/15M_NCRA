...
#define SOLAR_SEMIDIAM  960.           /*  arcseconds      */
...
#include "libvlba.h"
  ...
  void aberrat (tnowp, epsilonp, mean_posn, app_posn)
    double *tnowp;             /*  time relative to J2000, in Julian centuries*/
    double *epsilonp;          /*  mean obliquity of the ecliptic at t_now    */
    double mean_posn[3];     /*  dir-cosines of position to be aberrated    */
    double app_posn[3];      /*  (output) dir-cosines of aberrated position */
/*
  pointing calculations -- aberration and gravitational deflection
...
TEST STATUS:
  ...
  (End of test status for version 1.0, 3 April 86.)
 
++$  AUDIT TRAIL
    1.0   86apr03   D. King   Initial submission.
          90feb05   A. Pramesh Rao	changed variable sun[3] to
					ssun[3] to avoid clash with
					system defined sun on the SUN   
          93nov24   A. Pramesh Rao	changed calling sequence for tnow
					and epsilon from double to double *
					to make the routine compatible with 
					both cc and ANSI c
...
{ 
	...
	epsilon=*epsilonp,
        ...
        u,                   /*  Moons mean argument of latitude            */
        ...
        cos_sunl,
        sin_sunl,
        ssun[3],              /*  Sun's true position (direction cosines)    */
        ...
        ve2, ve3,            /*  intermediate values in computing ve        */
...
*/
    ...
    double
        rad_day = DEGREE/JUL_YEAR,     /*  Converts degrees per year
                                                 to radians per day         */
        mr = ASTR_UNIT/1000.,          /*  Astronomical unit in Kilometers  */
        ab_factr = 1000./(SEC_DAY * ARCSEC * C_LIGHT);    /*
                             **  Converts kilometers per day (velocity)
                             **        to arcsec (aberration)               */
    ...
    cnt = (6910.057 - 4.72e-4 * d) * sin(g) + 72.100 * sin(2. * g) +
                   1.054 * sin(3. * g);
 
      /*   Lunar perturbation and its derivatives  [Atkinson A8 - A12]      */
    lpl = 6.454 * sin(e) - 0.424 * sin(e - g1);
    lpr = 1336. * cos(e) -  133. * cos(e - g1);
 
    dlpr = - 284.26 * sin(e)         - 1.91 * sin(3.*e)
           -  16.31 * sin(e+g1)      - 2.03 * sin(e-g1)
           -   3.28 * sin(3.*e-g1)   + 3.22 * sin(e+g)
           -   7.04 * sin(e-g);
 
    dlpl =   1.3732 * cos(e)         + 0.0083 * cos(3.*e)
           + 0.0780 * cos(e+g1)      + 0.0065 * cos(e-g1)
           + 0.0160 * cos(3.*e-g1)   - 0.0147 * cos(e+g)
           + 0.0336 * cos(e-g);
    ...
    */
    ...
    ssun[1] = cos_sunl;
    ssun[2] = sin_sunl * cos_eps;
    ssun[3] = sin_sunl * sin_eps;
...
/*END-DEBUG*/
    ...
    */
    ...
    pr_dal = 5.9256e6 * (lpr * 1.e-8 + ppr * 1.e-9);           /* A19 */
    dpr = LOG_E_10 * mr * (dlpr * 1e-8 + dppr * 1e-9);         /* A22 */
    ...
    */
    ...
    bd = 0.008581 * sin(lj) + 0.001896 * sin(ls) + 0.000204;
      ...
      /* Eccentricity and perihelion (J2000)
      **           [Explan. Suppl. (1968), pg 48]                    */
...
/*debug*/   printf("aberrat:  C = %7.3f,      D = %7.3f\n", c, d); }
    ...
    */
    ...
    cos_sundist = dotpr3(ssun, mean_posn);
    ...
    */
    ...
    for (i = 0, norm_factr = 0.0; i < 3; i++)
    {
        app_posn[i] = mean_posn[i] + ve[i] +
                        mu_deflect * (mean_posn[i] * cos_sundist - ssun[i]);
