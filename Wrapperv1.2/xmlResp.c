  ...
  int i ; char tmpBuf[DATALEN];
    ...
    for (i=0; i < respPtr->data.numpkt; i++ ) {
        ...
        xmlAddChild(n, node[nParam]);
           ...
           bzero(tmpBuf,DATALEN);strcpy(tmpBuf, &respPtr->data.prmname[i]);
           xmlNewChild(node[nParam], NULL, BAD_CAST "name",BAD_CAST tmpBuf);
           bzero(tmpBuf,DATALEN);strcpy(tmpBuf, &respPtr->data.prmvalue[i]);
    ...
    switch (respPtr->event) {
         ...
         case ALARM        :   // Basic structure not matter
                              ...
                              xmlNewChild(n, NULL, BAD_CAST "message",BAD_CAST respPtr->message);
                               ...
                               // Form Alarm XML
                                 ...
                                 bzero(tmpBuf,DATALEN);strcpy(tmpBuf, "alarm_temp");
                                 ...
                                 bzero(tmpBuf,DATALEN);strcpy(tmpBuf, &respPtr->alarm.description);
         ...
         default            : if( respPtr->event > 12) {
                                    ...
                                    xmlNewChild(n, NULL, BAD_CAST "message",BAD_CAST respPtr->message);
                                      ...
                                      respPtr->event = 12; // Do not wait for the Final Response since corrupted packet received 20/3/2012
                                      // code
                                      bzero(tmpBuf,DATALEN);sprintf(tmpBuf,"%d",respPtr->code);
                                      xmlNewChild(n, NULL, BAD_CAST "code",BAD_CAST tmpBuf);
                                      // event
...
void formBasicXmlResp( xmlNodePtr n, BasicFlds *bptr) {
 ...
 extern char *xmlKeyWords[];
   ...
   char tmpBuf[DATALEN];
    ...
    bzero(tmpBuf,DATALEN);sprintf(tmpBuf,"%ld",bptr->seq);
...
/* Temporary commented as CMS giving problem 20/3/2012 - JPK
