...
#define _GNU_SOURCE
   ...
   ssize_t bytes= -1;
     ...
     if( (bytes = writen(*fd,(unsigned char*)devResp, devResponseSize)) != devResponseSize )
     {
       fprintf(stderr, "### Error: sendRespPkt()  Failed [%d]!! \n", bytes);
        ...
        fprintf(stderr, "sendRespPkt() Response send %d [%d] !!\n", bytes, devResponseSize);
        return 0;
     }
 }
 ...
 {           
  ...
  if ( (bytes = readn(*fd, (unsigned char *)devResp, devResponseSize)) != devResponseSize ) {
         fprintf(stderr, " WARNING : recvRespPkt() : Response not received [%d] !!\n", bytes);
       // Check machine is alive or not by ping routine
      return 1;                
  } else {                   
...
void printBasicFld(BasicFlds *bptr) {
    fprintf(stderr, "\n\t seq [%ld]  Subsystemid [%s]  \n\t ID [%s] CMDName [%s] \n\t TimeStamp [%s] \n",
       bptr->seq, bptr->subsysid, bptr->id, bptr->cmdname, bptr->timestamp);
...
{
         printBasicFld(&drptr->cmdelem);
         printDataPkt(&drptr->data);
         printAlarmPkt(&drptr->alarm);
...
*/
 ...
 reuse=1;
 if(setsockopt(sockfd, IPPROTO_TCP,TCP_NODELAY, &reuse,sizeof(reuse)) == ERROR)
 { perror("setsockopt_TCP_NODELAY"); return ERROR; }   
