...
char *xmlKeyWords[] = { "seq", "id", "name", "systemid", "username", "version", "timestamp", "priority", "timeout", "data", "param", "name", "value", "syntax" };
 ...
 {
     xmlChar *key; char argName[DATALEN], argData[DATALEN];
     int i=0;
     ...
     while (cur != NULL) 
     {
                i=0; bzero(&argName,DATALEN);  bzero(&argData,DATALEN); 
                ...
                if ((!xmlStrcmp(cur->name, (const xmlChar *)"param")))
                {
                     ...
                     if ( !xmlStrcmp(cur->name, (const xmlChar *)xmlKeyWords[Pname]) )  {
                           ...
                           if( (const char *) (key = xmlNodeListGetString(doc, cur->xmlChildrenNode, 1)) != NULL)
  ...
  int parseCmdStore(int xmlKey, char *xmlData, xmlDocPtr doc, xmlNodePtr cur)
  {
      ...
      switch(xmlKey)
      {
         ...
         case Username : if(xmlData != NULL)
                         strcpy(cmde->username,xmlData); break;
         case Priority : break;
         case Timeout :  break;
         case Data    :  parseCmd.data.numpkt = 0 ;
                         return  parseData(doc, cur);
                         break;
         case Syntx   : if(xmlData != NULL) 
                        if (strlen(xmlData) > SYNTXLEN) {
                          fprintf(stderr, "### ERROR xmlParse : parseCmdStore() Syntax Length Invalid\n");
                          return ERROR;
                        } else { strcpy(parseCmd.syntx,xmlData); }
             ...
             for( i= 0; i < TXE ; i++)
               {
                  ...
                  {
                       key = xmlNodeListGetString(doc, cur->xmlChildrenNode, 1);
                       if( parseCmdStore(i, key,doc,cur) == ERROR ) 
                       {
