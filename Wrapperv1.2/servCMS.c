...
// Sort Arguments as per the syntax

int sortArgs(parseCMSCmd *cptr)
{    DataPkt dPkt;
     short enarg[MAXDATA];
     char *fldSep;  char *fldChar;
     char strsyntx[SYNTXLEN],*ptr;
     int i, isort=0;

     if(cptr->data.numpkt)
     {
         for(i=0;i< cptr->data.numpkt;i++)enarg[i]=0;
         strcpy(strsyntx,cptr->syntx);
         fprintf(stderr, " sortArgs SNTYX:  %s \n",strsyntx);

         if( ( fldSep = ( char * ) getenv("FLD_SEPARATOR")) == NULL ) {
             fldChar = " ,";       
         } else { fldChar = fldSep; }

        ptr = (char *)strtok(&strsyntx,fldChar);
            // fprintf(stderr, " [%s] ",ptr);
        for(i=0;i<cptr->data.numpkt;i++) {
             if(enarg[i])continue;
             if( !strcasecmp(ptr,cptr->data.prmname[i]) ) {
                       //fprintf(stderr, "%d %s ",i,cptr->data.prmname[i]);
                  strcpy(dPkt.prmname[isort],cptr->data.prmname[i]);
                  strcpy(dPkt.prmvalue[isort],cptr->data.prmvalue[i]);
                  enarg[i]=1; isort++; break;
             }
        }
        while( (ptr = strtok(NULL,fldChar)) != NULL) {
                   //  fprintf(stderr, " [%s] ",ptr);
           for(i=0;i<cptr->data.numpkt;i++) {
                if(enarg[i])continue;
                if(strcasecmp(ptr,cptr->data.prmname[i]) == 0 ) {
                   //    fprintf(stderr, "%d %s ",i,cptr->data.prmname[i]);
                     strcpy(dPkt.prmname[isort],cptr->data.prmname[i]);
                     strcpy(dPkt.prmvalue[isort],cptr->data.prmvalue[i]);
                     enarg[i]=1; isort++; break;
       ...
       //for(i=0;i<cptr->data.numpkt;i++) { fprintf(stderr, " %d ",enarg[i]); }fprintf(stderr,"\n");




        // Syntax less args are more
        for(i=0; i< cptr->data.numpkt;i++) {
            if(!enarg[i]) {
                strcpy(dPkt.prmname[isort],cptr->data.prmname[i]);
                strcpy(dPkt.prmvalue[isort],cptr->data.prmvalue[i]);
        ...
        if(isort == cptr->data.numpkt) {
              fprintf(stderr, " isort %d numpkt %d \n", isort,cptr->data.numpkt);
              dPkt.numpkt= cptr->data.numpkt;
              memcpy(&cptr->data,&dPkt,(size_t)DataPktSize);
              for(i=0;i< cptr->data.numpkt;i++) {
                  fprintf(stderr, " ArgSorted  %s %s \n",cptr->data.prmname[i],cptr->data.prmvalue[i]);
              }
              return 0;
        }else { return ERROR; }
    }
...
// Form Response
   ...
   strcpy(devStat.cmdelem.timestamp,"0000-00-00T00:00:00+05:30");
...
*/
  ...
  if( srvflag == Success) {
     ...
     } else if ( sortArgs(&parseCmd)) {
             srvflag=xmlSyntxErr;
             fprintf(stderr, "### ERROR SyntxErr %s %s \n",parseCmd.cmdelem.subsysid ,subSysID); 
